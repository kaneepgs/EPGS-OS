export function createNarrativeEngine() {
  ...
  return Object.freeze({
    evaluate(_, { insights, recommendations, health }) {
      const topRecommendations = recommendations.slice(0, 3);
      const dailyBriefing = {
        headline: insights.topInsight.title,
        summary: `${insights.topInsight.executiveSummary} Next, leadership should focus on ${topRecommendations.map((item) => item.recommendation.toLowerCase()).join('; ')}.`,
      ...
      const weeklyBriefing = {
        summary: `${insights.executive[0]?.executiveSummary || ''} ${insights.executive[1]?.executiveSummary || ''}`.trim(),
